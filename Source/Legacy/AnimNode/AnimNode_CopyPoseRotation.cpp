...
#include "AnimNode_CopyPoseRotation.h"
#include "Animation/AnimInstanceProxy.h"
#include "ReferenceSkeleton.h"
#include "Animation/AnimCurveUtils.h"

#define LOCTEXT_NAMESPACE "AnimNode_CopyPoseRotation"

/////////////////////////////////////////////////////
// FAnimNode_CopyPoseRotation

FAnimNode_CopyPoseRotation::FAnimNode_CopyPoseRotation()
...
void FAnimNode_CopyPoseRotation::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
...
void FAnimNode_CopyPoseRotation::CacheBones_AnyThread(const FAnimationCacheBonesContext& Context)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_ANIMNODE(CacheBones_AnyThread)
	In.CacheBones(Context);
}

void FAnimNode_CopyPoseRotation::RefreshMeshComponent(USkeletalMeshComponent* TargetMeshComponent)
{
...
void FAnimNode_CopyPoseRotation::PreUpdate(const UAnimInstance* InAnimInstance)
{
	QUICK_SCOPE_CYCLE_COUNTER(FAnimNode_CopyPoseRotation_PreUpdate);
...
void FAnimNode_CopyPoseRotation::Update_AnyThread(const FAnimationUpdateContext& Context)
{
...
void FAnimNode_CopyPoseRotation::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	if (SourceMeshTransformArray.Num() > 0 && CurrentMesh)
	{
		...
		for (FCompactPoseBoneIndex PoseBoneIndex : OutPose.ForEachBoneIndex())
		{
			...
			const FBoneSourceSettings* Value = BoneMapToSourceSettings.Find(MeshBoneIndex);
			if (Value)
			{
...
void FAnimNode_CopyPoseRotation::GatherDebugData(FNodeDebugData& DebugData)
{
...
void FAnimNode_CopyPoseRotation::ReinitializeMeshComponent(USkeletalMeshComponent* NewSourceMeshComponent, USkeletalMeshComponent* TargetMeshComponent)
{
	CurrentlyUsedSourceMeshComponent.Reset();
	// reset source mesh
	...
	if (TargetMeshComponent && IsValid(NewSourceMeshComponent) && NewSourceMeshComponent->GetSkeletalMeshAsset())
	{
			...
			IsValid(TargetSkelMesh) && !TargetSkelMesh->HasAnyFlags(RF_NeedPostLoad))
		{
			...
			{
				FBoneSourceSettings SourceSettings;
...
#undef LOCTEXT_NAMESPACE //AnimNode_CopyPoseRotation
